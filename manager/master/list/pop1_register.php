...
$sql = "    
SELECT 
    r.temp_idx, r.influencer_idx, r.theme, r.title, r.sub_title,
    t.name as t_name, t.ico, t.sort
FROM 
    recipe_temp as r
LEFT OUTER JOIN
...
$stmt->bind_result($temp_idx, $influencer_idx, $theme, $title, $sub_title, $t_name, $ico, $sort);
$stmt->store_result();
...
$sql = "
    ...
    FROM 
        attachment_temp
    WHERE
        from_table_id = 'recipe' AND from_temp_id = '$temp'
    ORDER BY 
        ordering_index ASC";
           ...
           <ul>
               <li style="display:none;">
                   ...
                   <div class="profile_right">
                       <input type="text" name="influencer" id="influencer" value="<?=$_SESSION['idx']?>" maxlength="10" placeholder="" class="textbox" autofocus="" required="">
...
<script>
    ...
    $(document).on('click', '.ico_confirm4', function() {
        ...
        if($('.camera_pic').length < 3) {
            alert("세로 4:3비율 사진 3~10장을 등록 해주세요.");
            return false;
        }
        
        $.ajax({
            type: "POST",
            url : "./ajax_recipe_add.php",
            data: { 
                temp : '<?=$temp?>', 
            ...
            success : function(data, status, xhr) {
                location.href="./pop_register2.php?temp=<?=$temp?>";
            },
    ...
    $(document).on('click', '#pic_file', function() {
        ...
        var limit = 10 - $('.camera_pic').length;

        var url = "/fnc/multifile/index3.php?temp=<?=$temp?>&limit=" + limit;
