...
$sql = "
SELECT 
    r.temp_idx, r.influencer_idx, r.title, r.sub_title, r.tag, r.tip, r.youtube, date_format(r.create_date, '%y.%m.%d') as create_date, 
...
$stmt->bind_result($temp_idx, $influencer_idx, $title_recipe, $sub_title_recipe, $tag_recipe, $tip_recipe, $youtube, $create_date, $update_date, $influencer_name, $influencer_tag);
$stmt->store_result();
...
// 레시피 인터뷰 파일 불러오기
$sql = " SELECT idx, ordering_index, file_url, file_name, from_temp_id, from_table_id FROM attachment_temp WHERE from_table_id='recipe_interview' AND from_temp_id=? ";
$stmt = $conn->prepare($sql);
$stmt->bind_param('d', $temp);
$stmt->execute();
...
$stmt->bind_result($idx, $ordering_index, $file_url, $file_name, $from_temp_id, $from_table_id);
$stmt->store_result();
...
while($stmt->fetch())
{
    ...
    $interview_list[$list_idx]['from_temp_id'] = $from_temp_id;
...
// 임시 레시피 파일불러오기
$sql = " SELECT idx, ordering_index, file_url, file_name, from_temp_id, from_table_id FROM attachment_temp WHERE from_table_id='recipe' AND from_temp_id=? ";
$stmt = $conn->prepare($sql);
...
while($stmt->fetch())
{
    ...
    $attach_list[$list_idx]['from_temp_id'] = $from_temp_id;
...
$stmt->bind_result($ins_idx2, $amount2, $unit2, $name2);
$stmt->store_result();
...
while($stmt->fetch())
{
    $ing2[$list_idx]['ins_idx'] = $ins_idx2;
    $ing2[$list_idx]['amount'] = $amount2;
    $ing2[$list_idx]['unit'] = $unit2;
    $ing2[$list_idx]['name'] = $name2;
    $list_idx = $list_idx + 1;
}
$stmt->free_result();
$stmt->close();
          ...
          <div class="recipe_detail">
              <div class="influ_box">
                  <div class="influ_line">
                      <div class="recipe_sub"><?=$sub_title_recipe?></div>
                      <!-- 한줄만 보여야합니다 -->
                      <div class="recipename"><?=$title_recipe?></div>
                  </div>
                ...
                <div class="recipe_material">
                   <h7>[양념]</h7>
                    <ul>
                        ...
                        ?>
                        <li>
                            <div class="recipe_front"><?=$ing2[$i]["name"]?></div>
                            <div class="recipe_back"><?=$ing2[$i]["amount"].$ing[$i]["unit"]?></div>                            
                        </li>
                        <?php
                        }
                        ?>
                ...
                <div class="influ_recipe">
                    ...
                    <div class="tag_pic">
                      <ul>
                        ...
                        ?>
                          <li>
                              <a href="/recipe/pop_detail.php?ridx=<?=$other[$i]["recipe_idx"]?>" onclick="javascript:return false;">
...
<div id="container"> 
   <div class="sub_txt pop_txt">레시피 등록</div>
    ...
    <div class="register_btn">
          <div class="ico_confirm3"><a href="./pop_register4.php?temp=<?=$temp?>"><img src="/img/ico_arrow.svg">이전 페이지</a></div>
          <div class="ico_confirm2">등록하기<img src="/img/ico_arrow.svg"></div>
    </div>
</div>
