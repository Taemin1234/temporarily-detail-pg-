...
// 태그 삭제
if($count > 0) {
    $sql = "DELETE FROM recipe_join_tag WHERE recipe_idx = '$r_idx'";
...
for($i = 0; $i < $count; $i++) {
    ...
    if($tag_idx != "") {
        $sql = "INSERT INTO recipe_join_tag (recipe_idx, tag_idx, tag_num) VALUES ('$r_idx', '$tag_idx', '$tag_num')";
...
$sql = "UPDATE recipe SET tip = '$tip', youtube = '$youtube' WHERE recipe_idx = '$r_idx'";
...
foreach ($_FILES as $file)
{
    if ( $file['error'] == 0)
    {
        ...
        // 기존 DB 삭제
        $stmt = $conn->prepare("DELETE FROM attachment WHERE from_content_id = '$r_idx' AND from_table_id = 'recipe_interview'");
        $stmt->execute();
        $conn->commit();
        if($conn -> connect_error || $conn -> connect_errno || mysqli_stmt_error($stmt) || mysqli_stmt_errno($stmt))
        {
        ...
        // 디비 입력
        $stmt = $conn->prepare("INSERT INTO attachment (ordering_index, file_url, file_name, from_content_id, from_table_id) VALUES (?, ?, ?, ?, 'recipe_interview')");
        $stmt->bind_param('dssd', $order, $target_file, $filename, $r_idx);
        $stmt->execute();
        $conn->commit();
        if($conn -> connect_error || $conn -> connect_errno || mysqli_stmt_error($stmt) || mysqli_stmt_errno($stmt))
        {
